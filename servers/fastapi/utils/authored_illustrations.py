...
async def apply_authored_illustrations(
    ...
    semaphore = asyncio.Semaphore(_ILLUSTRATION_CONCURRENCY)

    async def fill_slide(index: int, html: str) -> str:
        ...
        for position, match in reversed(list(enumerate(matches))):
            replacement = (
                _resolve_slot(match.group(0), data_uri) if position == 0 else ""
            )
            result = result[: match.start()] + replacement + result[match.end() :]
        return result

    return list(
        await asyncio.gather(*(fill_slide(i, html) for i, html in enumerate(htmls)))
    )
