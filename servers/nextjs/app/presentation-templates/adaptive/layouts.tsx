...
import { cardGridDensity, comparisonDensity } from "./density";
...
export const ComparisonLayout: React.FC<{ blocks: AnyBlock[] }> = ({ blocks }) => {
  ...
  const d = comparisonDensity(cols);
  return (
    <div className="h-full w-full flex flex-col justify-center" style={{ gap: SECTION_GAP }}>
      {title && (
        <h1 data-block-id={title.id} style={headingStyle("var(--fs-h1, 3rem)")}>
          ...
          return (
            ...
            >
              <div
                ...
                className={`${d.headPad} font-bold`}
                style={{ background: PRIMARY, color: ON_PRIMARY, fontSize: d.headFs }}
              >
                {col.heading}
              </div>
              <ul className={`flex flex-col ${d.listClass}`}>
                {items.map((it) => (
                  <li key={it.id} data-block-id={it.id} className={`flex items-start ${d.itemGap}`} style={{ color: TEXT_COLOR, fontSize: d.itemFs }}>
