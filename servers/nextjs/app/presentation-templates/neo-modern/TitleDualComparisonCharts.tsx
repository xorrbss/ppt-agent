    ...
    .array(
      z.object({
        ...
        subHeading: z
          ...
          .max(20)
          .optional()
          .describe("An optional badge or subtitle"),
        footerText: z
          .string()
          .max(25)
          .describe("The text displayed at the bottom of the chart area"),
        chartType: chartTypeEnum.describe("Type of chart to display"),
        chart: z.object({
          columns: z
            .array(z.string())
            .max(5)
            .describe("The labels for the X-axis categories"),
          rows: z
            .array(
              z
                .array(
                  z.object({
                    label: z
                      .string()
                      .max(30)
                      .describe("The name of the data series"),
                    value: z
                      .number()
...
const dynamicSlideLayout: React.FC<{
  ...
  const renderChart = (card: NonNullable<typeof comparisonCards>[number]) => {
    ...
    const chartData =
      card.chart?.columns?.map((col, cIdx) => ({
        name: col,
            ...
            >
              ...
              <Legend
                ...
                formatter={(value: any, entry: any) => {
                  if (entry.dataKey === "series1")
                    return chartData[0]?.series1Name;
                  if (entry.dataKey === "series2")
                    return chartData[0]?.series2Name;
                  return value;
                }}
              />
              <Line
        ...
        return (
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={chartData}
              layout="vertical"
              margin={{ top: 40, right: 10, left: 60, bottom: 20 }}
              barGap={0}
            >
              <CartesianGrid
                horizontal={false}
              ...
              <Bar
                ...
                fill={graphColors(0)}
                barSize={30}
                radius={hasSeries2 ? undefined : [0, 4, 4, 0]}
              >
                <LabelList
                  dataKey="series1"
                  position="center"
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar
                ...
                fill={graphColors(0)}
                barSize={50}
                radius={hasSeries2 ? undefined : [4, 4, 0, 0]}
              />
              {hasSeries2 && (
                <Bar
                  dataKey="series2"
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 15, right: 30, left: 0, bottom: 0 }}>
              ...
              <XAxis
                type="number"
                dataKey="x"
                name={chartData[0]?.series1Name}
                {...axisProps}
              />
              <YAxis
                type="number"
                dataKey="y"
                name={chartData[0]?.series2Name}
                {...axisProps}
              />
              <Tooltip
                content={<CustomTooltip />}
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-white z-20 mx-auto overflow-hidden flex flex-col p-10 "
        style={{
      ...
      >
        ...
        <div className="w-full text-center mb-10">
          <h1
            className="text-[42.7px]  font-bold tracking-[-1.5px]"
        ...
        <div className=" flex-1 flex gap-8 justify-center  px-4">
          {comparisonCards?.map((card, idx) => {
            return (
              <div
                key={idx}
                className="flex-1  bg-white border border-[#F0F0F2] rounded-lg p-4 flex flex-col relative"
                style={{
                  backgroundColor: "var(--card-color,#FFFFFF)",
