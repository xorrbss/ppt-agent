...
export const Schema = z.object({
  ...
  chart: z
    .object({
      ...
      colorPalette: z
        .enum(["vibrant", "ocean", "professional"])
        .default("vibrant"),
    })
...
const ChartRenderer: React.FC<{
  ...
  const graphColors = (index: number, serieColor?: string) => {
    const fallback =
      serieColor || DEFAULT_CHART_COLORS[index % DEFAULT_CHART_COLORS.length];
            ...
            {chart.series.map((serie, index) => (
              ...
              >
                <LabelList
                  dataKey={serie.name}
                  position="top"
                  fill="var(--background-text,#101828)"
          ...
          >
            ...
            <defs>
              {chart.series.map((serie, index) => (
                <linearGradient
                  key={serie.name}
                  id={`metrics-gradient-${serie.name}`}
                  x1="0"
                  y1="0"
                  x2="0"
                  y2="1"
                >
                  <stop
                    offset="5%"
                    stopColor={graphColors(index, serie.color)}
                    stopOpacity={0.4}
                  />
                  <stop
                    offset="95%"
...
const dynamicSlideLayout: React.FC<{ data: Partial<SlideData> }> = ({
  data,
}) => {
      ...
      >
        ...
        <div className="mb-8 flex justify-between">
          <div className="max-w-[400px]">
            <h1
              className="text-[42.7px] font-bold leading-tight mb-4 tracking-[-2px]"
          ...
          <p
            className="text-[17.8px] max-w-[560px] leading-[1.6]"
        ...
        <div className=" flex justify-between items-center gap-6 w-full">
          <div
            className=" flex-1 overflow-hidden border  rounded-xl p-4 shadow-sm"
            style={{
              backgroundColor: "var(--card-color,#F0F0F2)",
          ...
          >
            ...
            <div className="h-[400px] min-h-0 overflow-hidden">
              <ChartRenderer
                chart={chart ?? { categories: [], series: [], type: "bar" }}
              />
            </div>
          </div>
          <div className="  flex-1 flex flex-wrap items-center  justify-start gap-x-8 gap-y-12 pl-4">
            {metrics?.map((metric, index) => (
              <div key={index} className=" max-w-[245px] w-full ">
                <div
                  className="text-[45px] font-medium leading-none"
