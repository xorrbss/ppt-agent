...
export const layoutName = '멀티 차트 그리드와 글머리가 있는 제목 설명';
export const layoutDescription = '제목과 설명, 최대 4개의 글머리, 그리고 반응형 그리드에 자동 배치되는 1~4개의 차트로 구성된 대시보드 레이아웃입니다. 막대(세로, 가로, 그룹, 누적, 클러스터, 발산), 선, 영역, 파이, 도넛, 산점도 차트를 지원합니다.';
...
export const Schema = z.object({
    ...
    bullets: z.array(z.string().max(80)).max(6).default([
        '파이프라인 커버리지가 목표의 3배를 초과',
        'CAC 회수 기간 6개월 이내',
        '엔터프라이즈 전환율 전분기 대비 개선',
        '확장 매출이 성장을 견인',
        '모든 코호트에서 유지율 95% 이상',
        '이번 분기 예측 정확도 개선',
    ]).meta({
    ...
    return (
        <>
            ...
            >
                ...
                {/* Main Content */}
                <div className="relative h-full min-h-0 overflow-hidden px-10 pt-10 pb-8 flex flex-col z-10">
                    ...
                    <div className="flex-1 flex gap-8 min-h-0 overflow-hidden">
                        ...
                        <div className={`flex-1 min-h-0 grid ${gridLayout.className} auto-rows-fr gap-4 overflow-hidden`}>
                            {charts.map((chart, index) => (
                                <div
                                    key={index}
                                    className="backdrop-blur-sm rounded-xl border border-gray-100/80 shadow-sm flex min-h-0 flex-col overflow-hidden"
