/**
 * Neo-standard layout: title, description, up to 4 metrics, and 1–6 charts in a responsive grid.
 */
...
export const layoutName = '제목 설명 멀티 차트 그리드 + 지표';
export const layoutDescription = '제목, 설명, 최대 4개의 KPI 지표와 1~6개의 자동 배치 차트를 갖춘 neo-standard 대시보드입니다. 막대(세로, 가로, 그룹, 누적, 클러스터, 발산), 선, 영역, 파이, 도넛, 산점도 차트를 지원합니다.';
...
export const Schema = z.object({
    ...
    metrics: z.array(z.object({
        ...
        label: z.string().max(24).default('파이프라인'),
    })).max(4).default([
    ...
    charts: z.array(ChartItemSchema).min(1).max(6).default([
        ...
        { title: '제품 비교', type: 'bar-clustered', data: [{ name: '1분기', values: { '제품 A': 45, '제품 B': 62 } }, { name: '2분기', values: { '제품 A': 58, '제품 B': 71 } }, { name: '3분기', values: { '제품 A': 72, '제품 B': 65 } }], series: ['제품 A', '제품 B'], colorPalette: 'vibrant' },
        { title: '고객 피드백', type: 'bar-diverging', data: [{ name: '품질', positive: 78, negative: 22 }, { name: '서비스', positive: 65, negative: 35 }, { name: '가격', positive: 42, negative: 58 }], series: ['만족', '불만족'], colorPalette: 'professional' },
    ]),
...
const CustomTooltip = ({ active, payload, label }: any) => {
    if (active && payload && payload.length) {
        return (
            <div className="bg-white/95 backdrop-blur-sm border rounded-lg shadow-lg px-3 py-2" style={{ backgroundColor: 'var(--card-color, #ffffff)', borderColor: 'var(--stroke, #E5E5E5)' }}>
                <p className="text-[10px] font-semibold mb-1" style={{ color: 'var(--background-text, #000)' }}>{label}</p>
                {payload.map((entry: any, index: number) => (
                    <p key={index} className="text-[9px]" style={{ color: 'var(--background-text, #000)' }}>
...
const MiniChartRenderer: React.FC<{
    ...
    const axisProps = {
        tick: { fill: 'var(--background-text, #7f8491)', fontSize: 9, fontWeight: 500, fontFamily: 'Open Sans Regular' },
            ...
            return (
                <ResponsiveContainer width="100%" height="100%">
                    <AreaChart data={data} margin={{ top: 10, right: 10, left: -10, bottom: 5 }}>
                        ...
                        <Tooltip content={<CustomTooltip />} cursor={{ fill: 'transparent' }} />
                        <defs>
                            <linearGradient id={`areaGrad-std-${chart.title}`} x1="0" y1="0" x2="0" y2="1">
                                <stop offset="5%" stopColor={graphColors(0)} stopOpacity={0.4} />
                                <stop offset="95%" stopColor={graphColors(0)} stopOpacity={0.05} />
                            </linearGradient>
                        </defs>
                        <Area type="monotone" dataKey="value" stroke={graphColors(0)} strokeWidth={2} fill={`url(#areaGrad-std-${chart.title})`} />
                    </AreaChart>
                </ResponsiveContainer>
            );
    ...
    return (
        <>
            ...
            >
                ...
                <div className="relative w-full h-full min-h-0 overflow-hidden p-[60px] pt-[50px] flex flex-col">
                        ...
                        {hasMetrics && (
                            <div className="mt-4 grid grid-cols-4 gap-3">
                                {metrics.map((metric, index) => (
                                    ...
                                    >
                                        <div className="text-[18px] font-semibold leading-tight" style={{ color: 'var(--background-text,#000)' }}>
                                            {metric.value}
                                        </div>
                                        <div className="text-[10px] font-medium mt-1 opacity-80" style={{ color: 'var(--background-text,#000)' }}>
                                            {metric.label}
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className={`flex-1 min-h-0 grid ${gridLayout.className} auto-rows-fr gap-4 overflow-hidden`}>
                        {charts.map((chart, index) => (
                            <div
                                key={index}
                                className="rounded-[6px] border flex min-h-0 flex-col overflow-hidden"
                                style={{ borderColor: 'var(--stroke,#F0F0F2)', backgroundColor: 'var(--card-color,#FFFFFF)' }}
                            >
