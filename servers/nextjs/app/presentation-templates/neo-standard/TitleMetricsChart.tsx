  ...
  chart: z
    .object({
      title: z
        .string()
        .max(20)
        .describe("The text in the green highlight box above the chart"),
      topLabel: z
        .string()
        .max(20)
        .describe("Secondary label at the top right of the chart area"),
      bottomLabel: z
        .string()
        .max(20)
        .describe("Secondary label at the bottom right of the chart area"),
      chartType: chartTypeEnum.describe("Type of chart to display"),
      seriesNames: z
        .array(z.string())
        .max(2)
        .describe("The names of the two data series for the legend"),
      data: z
        .array(
          z.object({
...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div
        className="bg-white/95 backdrop-blur-sm border border-gray-200 rounded-lg shadow-lg px-3 py-2"
        style={{
...
const dynamicSlideLayout: React.FC<{
  ...
  const renderChart = () => {
    const hasValue2 =
      chart?.data?.some((row) => (row.value2 ?? 0) > 0) ?? false;
    ...
    const axisProps = {
      tick: {
        fill: "var(--background-text, #7f8491)",
        fontSize: 11,
        fontWeight: 500,
      },
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Line
                ...
                strokeWidth={3}
                dot={{ fill: "var(--graph-0, #1F8A2E)", r: 5 }}
              />
              {hasValue2 && (
                <Line
                  type="monotone"
                  dataKey="value2"
                  stroke="var(--graph-1, #A8D9A8)"
                  strokeWidth={3}
                  dot={{ fill: "var(--graph-1, #A8D9A8)", r: 5 }}
                />
              )}
            </LineChart>
          </ResponsiveContainer>
        ...
        return (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <CartesianGrid
                horizontal={false}
                stroke="var(--background-text, #E0E0E0)"
              />
              ...
              <Bar
                dataKey="value1"
                stackId="a"
                fill="var(--graph-0, #1F8A2E)"
                radius={hasValue2 ? undefined : [0, 4, 4, 0]}
              >
                <LabelList
                  dataKey="value1"
                  position="center"
                  fill="var(--primary-text, #FFFFFF)"
                  style={{ fontSize: "14px", fontFamily: "Open Sans Regular" }}
                />
              </Bar>
              {hasValue2 && (
                <Bar
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar
                ...
                stackId="stack"
                fill="var(--graph-0, #1F8A2E)"
                radius={hasValue2 ? undefined : [4, 4, 0, 0]}
              />
              {hasValue2 && (
                <Bar
                  dataKey="value2"
            ...
            >
              ...
              >
                <LabelList
                  dataKey="value1"
                  position="top"
                  fill="var(--background-text, #000)"
                  style={{ fontSize: "11px", fontFamily: "Open Sans Regular" }}
                />
              </Bar>
              {hasValue2 && (
                <Bar
        ...
        return (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <ReferenceLine
                x={0}
                stroke="var(--background-text, #9CA3AF)"
                strokeWidth={1}
              />
              <Tooltip
                content={<CustomTooltip />}
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-white z-20 mx-auto overflow-hidden flex flex-row p-[60px] gap-[40px]"
        style={{
      ...
      >
        ...
        <div className="flex flex-col basis-[42%] justify-start">
          <h1
            className="text-[42.7px] text-black  font-bold leading-[1.1] mb-[20px] tracking-[-1.6px]"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {title}
          </h1>
          <p
            className="text-[16px] text-black  leading-[1.6] mb-[40px]"
            style={{ color: "var(--background-text,#000000)" }}
          >
            {description}
          </p>
          <div className="grid grid-cols-2 gap-[25px]">
            {metrics?.map((metric, index) => (
              <div
                key={index}
                className="bg-[#1F8A2E] rounded-[8px] p-[20px] flex flex-col items-center justify-center text-white text-center shadow-md min-h-[150px]"
                style={{ backgroundColor: "var(--primary-color,#1F8A2E)" }}
              >
                <div
                  className="text-[39px]  leading-[1]"
                  style={{ color: "var(--primary-text,#FFFFFF)" }}
                >
                  {metric.value}
                </div>
                <div
                  className="text-[17.8px]  mt-[5px] leading-[1.2]"
                  style={{ color: "var(--primary-text,#FFFFFF)" }}
                >
                  {metric.label}
                </div>
        ...
        {/* Right Column */}
        <div
          className="flex flex-col basis-[58%] bg-white border-[1.3px] border-[#F0F0F2] rounded-[6px] p-[20px]"
          style={{
        ...
        >
          <div className="flex justify-between items-end mb-[20px]">
            <div
              className="bg-[#1F8A2E] px-[20px] py-[8px]"
              style={{ backgroundColor: "var(--primary-color,#1F8A2E)" }}
            >
              <span
                className="text-white  font-bold text-[28.4px]"
                style={{ color: "var(--primary-text,#FFFFFF)" }}
              >
                {chart?.title}
              </span>
            ...
            <div
              className="text-black  text-[18.7px]"
              style={{ color: "var(--background-text,#000000)" }}
            >
              {chart?.topLabel}
            </div>
          ...
          <div className="flex-grow min-h-0 overflow-hidden relative">
                ...
                {chart?.seriesNames
                  ...
                  ?.map((name, i) => (
                    <div key={i} className="flex items-center gap-[8px]">
                      ...
                      />
                      <span
                        className="text-[12px] text-black font-['Open_Sans_Regular']"
                        style={{ color: "var(--background-text,#000000)" }}
                      >
                        {name}
                      </span>
            ...
            <div
              className="absolute bottom-[0px] right-[20px] text-black font-['Playfair_Display_Regular'] text-[16px]"
              style={{ color: "var(--background-text,#000000)" }}
            >
              {chart?.bottomLabel}
            </div>
