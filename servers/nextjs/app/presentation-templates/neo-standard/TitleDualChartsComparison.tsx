    ...
    .array(
      z.object({
        heading: z
          .string()
          .max(10)
          .describe("The heading for this comparison section"),
        tagline: z
          .string()
          .max(10)
          .optional()
          .describe("An optional tagline or status text for the section"),
        footerLabel: z
          .string()
          .max(15)
          .describe("Label displayed at the bottom of the chart area"),
        chartType: chartTypeEnum.describe("Type of chart to display"),
        legendItems: z
          .array(z.string().max(10))
          .max(2)
          .describe("Labels for the two data series in the legend"),
        graph: z
          .object({
            rows: z
              .array(
                z.object({
                  label: z
                    .string()
                    .describe("The label for the individual category"),
...
const dynamicSlideLayout: React.FC<{
            ...
            >
              ...
              {hasValue2 && (
                <Line
                  type="monotone"
                  dataKey="value2"
                  stroke="var(--graph-1, #A9D9B1)"
                  strokeWidth={3}
                  dot={{ fill: "var(--graph-1, #A9D9B1)", r: 5 }}
                />
              )}
            </LineChart>
          </ResponsiveContainer>
        ...
        return (
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              <CartesianGrid
                horizontal={false}
                stroke="var(--background-text, #E5E5E5)"
              />
          ...
          <ResponsiveContainer width="100%" height="100%">
            <ScatterChart margin={{ top: 15, right: 30, left: 0, bottom: 0 }}>
              ...
              <XAxis
                type="number"
                dataKey="x"
                name={section?.legendItems?.[0]}
                {...axisProps}
              />
              <YAxis
                type="number"
                dataKey="y"
                name={section?.legendItems?.[1]}
                {...axisProps}
              />
              <Tooltip
                content={<CustomTooltip />}
  ...
  return (
    <>
      ...
      />
      <div
        className="relative w-full rounded-sm max-w-[1280px] shadow-lg max-h-[720px] aspect-video bg-white z-20 mx-auto overflow-hidden flex flex-col"
        style={{
      ...
      >
        {/* Main Title */}
        <div className="w-full flex justify-center mt-[60px] mb-[40px] shrink-0">
          <h1
            className="text-[42.7px] text-[#000000]  font-bold"
            style={{
              letterSpacing: "-1.6px",
        ...
        <div className="flex flex-1 min-h-0 justify-center gap-[25px] overflow-hidden px-[36px]">
          {comparisonSections?.map((section, idx) => (
            <div
              key={idx}
              className="relative flex min-h-0 w-[590px] flex-col overflow-hidden border-[1.3px] rounded-[6px] p-6"
              style={{ borderColor: "var(--stroke,#F0F0F2)" }}
            >
              {/* Heading with background */}
              <div
                className="absolute left-[15px] top-[-10px] bg-[#1F8A2E] px-4 py-2"
              ...
              {section?.tagline && (
                <div className="text-right w-full mb-4">
                  <span
                    className="text-[19px] text-[#1F8A2E] "
                        ...
                        <div key={lIdx} className="flex items-center gap-2">
                          <div
                            className={`w-3 h-3 rounded-full ${
                              lIdx === 0
                                ? "bg-var(--graph-0, #1F8A2E)"
                                : "bg-var(--graph-1, #A9D9B1)"
                            }`}
                          />
                          <span
              ...
              {/* Chart Area */}
              <div
                className={`flex-grow min-h-0 w-full overflow-hidden ${
                  (section?.chartType === "pie" ||
                    section?.chartType === "donut") &&
                  !section?.tagline
                    ? "mt-8"
              ...
              <div className="absolute right-[20px] bottom-[5px]">
                <span
                  className="text-[16px] text-[#1F8A2E] "
                  style={{ color: "var(--primary-color,#1F8A2E)" }}
                >
                  {section?.footerLabel}
                </span>
