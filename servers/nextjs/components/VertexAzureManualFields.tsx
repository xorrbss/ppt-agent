...
function hasAzureAdvancedContent(config: LLMConfig): boolean {
  return (
    !!(config.AZURE_OPENAI_MODEL || '').trim() || !!(config.AZURE_OPENAI_BASE_URL || '').trim()
  );
}
...
export default function VertexAzureManualFields({
  ...
  const [azureAdvancedOpen, setAzureAdvancedOpen] = useState(() =>
    hasAzureAdvancedContent(llmConfig),
  );
  ...
  useEffect(() => {
    if (hasAzureAdvancedContent(llmConfig)) setAzureAdvancedOpen(true);
  }, [llmConfig.AZURE_OPENAI_MODEL, llmConfig.AZURE_OPENAI_BASE_URL]);
        ...
        <>
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                onPatch(
                  v.trim()
                    ? { AZURE_OPENAI_ENDPOINT: v, AZURE_OPENAI_BASE_URL: '' }
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              onChange={(e) => onPatch({ AZURE_OPENAI_DEPLOYMENT: e.target.value })}
              className={inputClass}
          ...
          <Collapsible open={azureAdvancedOpen} onOpenChange={setAzureAdvancedOpen}>
            <CollapsibleTrigger asChild>
              <button
                type="button"
                className="flex w-full min-w-0 items-center justify-between gap-2 rounded-lg border border-gray-200 bg-[#F9F9FA] px-3 py-2.5 text-left text-sm font-medium text-gray-800 transition-colors hover:bg-gray-100"
                ...
                <div className="space-y-1.5">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      onPatch(
                        v.trim()
                          ? { AZURE_OPENAI_BASE_URL: v, AZURE_OPENAI_ENDPOINT: '' }
                          : { AZURE_OPENAI_BASE_URL: v },
                      );
                    }}
                    className={inputClass}
                    placeholder="https://..."
